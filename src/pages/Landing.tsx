          ...
          <div className="flex gap-4 overflow-x-auto pb-3 scrollbar-hide snap-x snap-mandatory md:grid md:grid-cols-3 md:overflow-visible md:pb-0">
            {(blogPosts ?? []).slice(0, 2).map((post, i) => (
              <motion.div
                ...
                variants={fadeUp}
                custom={i + 1}
                className="min-w-[280px] snap-start md:min-w-0 flex"
              >
                <Link
                  to={`/blog/${post.slug}`}
                  className="flex flex-col w-full rounded-xl border border-border bg-card/80 backdrop-blur-sm overflow-hidden hover:shadow-lg hover:border-primary/10 transition-all duration-300 group"
                >
                  ...
                  <div className="p-4 flex-1 flex flex-col">
                    ...
                    <p className="text-xs text-muted-foreground mt-auto pt-3">{post.published_at ? format(new Date(post.published_at), "MMM yyyy") : ""}</p>
                  </div>
                </Link>
              </motion.div>
            ))}
