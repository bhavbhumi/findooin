...
describe("useLeaderboard (renderHook)", () => {
  ...
  it("should call get_leaderboard RPC with default limit", async () => {
    ...
    await waitFor(() => {
      expect(mockRpc).toHaveBeenCalledWith("get_leaderboard", { p_limit: 20, p_offset: 0 });
    });
  });
  ...
  it("should call get_leaderboard RPC with custom limit", async () => {
    ...
    await waitFor(() => {
      expect(mockRpc).toHaveBeenCalledWith("get_leaderboard", { p_limit: 50, p_offset: 0 });
    });
  });
});
...
describe("useWeeklyChallenges (renderHook)", () => {
  ...
  it("should query weekly_challenges and user_challenge_progress", async () => {
    renderHook(() => useWeeklyChallenges("user-1"), { wrapper: createWrapper() });

    await waitFor(() => {
      expect(mockFrom).toHaveBeenCalledWith("weekly_challenges");
    });
  });
});
